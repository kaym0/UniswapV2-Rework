...
contract RewardVault is ERC20, IERC4626 {
     ...
     */
    modifier getReward(address owner) {
     ...
     */
    function deposit(uint256 assets, address receiver) public getReward(_msgSender()) override returns (uint256 shares) {
        require((shares = previewDeposit(assets)) != 0, "No Shares");

        assetToken.transferFrom(_msgSender(), address(this), assets);

        _mint(receiver, shares);

        //emit Deposit(_msgSender(), receiver, shares);
    }

    function depositAll(address owner) public getReward(_msgSender()) returns (uint256) {
        return deposit(assetToken.balanceOf(owner), owner);
    }
     ...
     *  @return shares - The amount of shares received
     */
    function withdraw(uint256 assets, address receiver, address owner) public getReward(_msgSender()) override returns (uint256 shares) {
    ...
    function withdrawAll(address owner, address receiver) public getReward(_msgSender()) returns (uint256) {
     ...
     */
    function claimReward(address owner) public returns (uint256 rewardAmount) {
        uint256 shares = convertToShares(balanceOf(owner));
        ...
        emit Claim(owner, owner, rewardAmount);
    }
